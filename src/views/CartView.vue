<template>
  <div class="p-8 max-w-[1400px] mx-auto space-y-8 min-h-screen bg-[#0f1111] text-gray-100">
    <h1 class="text-3xl font-black text-white">{{ lang === 'en' ? 'Your Cart' : 'Shporta Juaj' }}</h1>
    ...
    <div v-if="cart.length > 0" class="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div class="flex items-center gap-4">
            ...
            <div>
              ...
              <p class="text-xs text-gray-500 mt-1 flex flex-wrap items-center gap-1.5">
                <span>{{ lang === 'en' ? 'Category:' : 'Kategoria:' }} {{ item.category }}</span>
                ...
                <span v-if="item.selectedVariant" class="text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded text-[10px] font-bold">
                  {{ lang === 'en' ? 'Option:' : 'Opsioni:' }} {{ item.selectedVariant }}
                </span>
              </p>
            </div>
          </div>
      ...
      <div class="lg:col-span-1">
        <div class="p-6 rounded-2xl border border-gray-800 bg-gray-900/20 backdrop-blur-sm space-y-6 sticky top-24">
          <h3 class="text-lg font-bold text-white border-b border-gray-800 pb-3">{{ lang === 'en' ? 'Order Summary' : 'Përmbledhja e Porosisë' }}</h3>

          <div class="space-y-3">
            <label class="block text-xs font-bold uppercase text-gray-400 tracking-wider">{{ lang === 'en' ? 'Delivery Notes' : 'Shënimet e Dërgesës' }}</label>
            <input v-model="customerName" type="text" :placeholder="lang === 'en' ? 'Full name' : 'Emri dhe Mbiemri'" class="w-full bg-gray-950 border border-gray-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500 transition" />
            <input v-model="customerCity" type="text" :placeholder="lang === 'en' ? 'City (e.g. Prizren)' : 'Qyteti (p.sh: Prizren)'" class="w-full bg-gray-950 border border-gray-800 rounded-xl px-4 py-2.5 text-xs text-white focus:outline-none focus:border-blue-500 transition" />
          </div>
          ...
          <div class="space-y-3 text-sm border-t border-gray-800/60 pt-4">
            ...
            <div class="flex justify-between text-gray-400">
              ...
              <span class="font-mono text-emerald-400">{{ shippingFee === 0 ? (lang === 'en' ? 'Free' : 'Falas') : shippingFee + ' €' }}</span>
            </div>
          ...
          >
            <span>💬</span> {{ lang === 'en' ? 'Order through WhatsApp' : 'Porosit përmes WhatsApp' }}
          </button>
          
          <p class="text-[10px] text-gray-500 text-center">
            {{ lang === 'en' ? 'When you confirm the order, product details are sent directly to the shop’s WhatsApp line.' : 'Kur të konfirmoni porosinë, detajet e produktit dërgohen direkt në linjën WhatsApp të dyqanit përkatës.' }}
          </p>
        </div>
      </div>
    ...
    <div v-else class="text-center py-20 border border-dashed border-gray-800 rounded-3xl bg-gray-900/5 max-w-xl mx-auto">
      ...
      <h2 class="text-xl font-bold text-white mt-6">{{ lang === 'en' ? 'Your cart is empty' : 'Shporta juaj është e zbrazët' }}</h2>
      <p class="text-gray-400 text-sm mt-2 max-w-sm mx-auto">{{ lang === 'en' ? 'There are no items in your cart. Browse our shops to find the best deals.' : 'Nuk ka asnjë produkt në shportë. Eksploroni dyqanet tona për të gjetur ofertat më të mira.' }}</p>
...
import { cart, saveCartToStorage, currentLang } from '../store/productStore'
